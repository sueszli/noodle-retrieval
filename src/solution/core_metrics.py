...
def load_qrels(path, is_ds=False):
    # if you use DawidSkene label aggregation, remove the first line of the file containing task 0
    # (was already done for our project)
    with open(path, 'r', encoding="utf-8") as file:
        qids_to_relevant_passageids = {}
        stripped = False
        for line in file:
            try:
                # different splitting when DawidSkene is used. This is needed because of the output format provided
                # by the aggregation
                if is_ds:
                    line = line.replace("\"", "").strip().replace(', ', '\t').split()
                    line.insert(1, "Q0")

                    # in the done aggregation, query and document ids had swapped positions (was a mistake)
                    # now we switch to avoid errors
